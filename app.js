...
function collectQuestions() {
  ...
  document.querySelectorAll('#questions-builder .qb-card').forEach(function(card) {
    ...
    if (type === 'mcq') {
      ...
      ceQuestions.push({ type:'mcq', text:text, options:opts, answer:ans, marks:marks });
    } else if (type === 'tf') {
      var tfR = card.querySelector('input[type="radio"]:checked');
      if (!tfR) { valid = false; return; }
      ceQuestions.push({ type:'tf', text:text, answer: tfR.value === 'true', marks:marks });
    } else if (type === 'essay') {
      var key = (card.querySelector('[data-field="answerKey"]').value || '').trim();
      ceQuestions.push({ type:'essay', text:text, answerKey:key, marks:marks });
    }
  });
